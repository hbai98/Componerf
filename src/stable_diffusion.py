...
class StableDiffusion(nn.Module):
    def __init__(self, device, model_name='CompVis/stable-diffusion-v1-4',concept_name=None, latent_mode=True):
        ...
        self.us = self.linear_us(self.num_train_timesteps)
        self.M = self.num_train_timesteps
        self.scale = 100
    ...
    def linear_us(self, M=1000):
        assert M == 1000
        b_start = 0.00085
        b_end = 0.0120
        bs = np.linspace(b_start**0.5, b_end**0.5, M, dtype=np.float64)**2
        a_s = np.cumprod(1 - bs)
        us = np.sqrt((1 - a_s) / a_s)
        us = us[::-1]
        return us
