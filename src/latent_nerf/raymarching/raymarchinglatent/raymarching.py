...
try:
    import _raymarchinglatent as _backend
except ImportError:
...
class _near_far_from_aabb(Function):
    ...
    def forward(ctx, rays_o, rays_d, aabb, min_near=0.2):
        ...
        nears = torch.zeros(N, dtype=rays_o.dtype, device=rays_o.device)
        fars = torch.zeros(N, dtype=rays_o.dtype, device=rays_o.device)
...
class _sph_from_ray(Function):
    ...
    def forward(ctx, rays_o, rays_d, radius):
        ...
        coords = torch.zeros(N, 2, dtype=rays_o.dtype, device=rays_o.device)
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, deltas, rays, T_thresh=1e-4):
        ...
        image = torch.zeros(N, 3 + 1, dtype=sigmas.dtype, device=sigmas.device)
