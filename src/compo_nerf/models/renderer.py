...
class NeRFRenderer(nn.Module):
    def __init__(self, cfg:RenderConfig, latent_mode:bool=True):
        ...
        self.density_init = cfg.density_init
        self.density_bitfield_init = cfg.density_bitfield_init
        if self.cuda_ray:
        ...
        if self.cuda_ray:
            # density grid
            density_grid = self.density_init*torch.ones([self.cascade, self.grid_size ** 3]) # [CAS, H * H * H]
            density_bitfield = self.density_bitfield_init*torch.ones(self.cascade * self.grid_size ** 3 // 8, dtype=torch.uint8) # [CAS * H * H * H // 8]
